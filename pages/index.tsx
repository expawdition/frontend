...
export default function Home({ dbTrips, error }: { dbTrips: any; error: any }) {
	...
	return (
		<>
			<Head>
				<title>Expawdition</title>
				<meta name='description' content='Expawdition' />
			...
			<Flex className={styles.header} justifyContent='space-between'>
				...
				<Flex className={styles.headerright} borderRadius='100px' borderColor='#1a365d' backgroundColor='white'>
					<Image src={Nugget} alt='profile picture' width={34} height={34} className={styles.roundedImage} />
				</Flex>
			</Flex>
			...
			>
				<Flex direction='row' width='1080px' justifyContent='space-between'>
					<Box paddingTop='100px'>
						<Text fontSize='3xl' lineHeight='9' fontWeight='bold' color='blue.900' marginBottom='16px'>
							{' '}
							Hello, Alex!
						</Text>
						<Text fontSize='5xl' lineHeight='1' fontWeight='extrabold' color='blue.900' marginBottom='16px'>
							{' '}
			...
			<main className={`${styles.main} ${inter.className}`}>
				<div className={styles.grid}>
					<Flex direction='column' minWidth='max-content'>
						<Tabs className={styles.tabs} variant='soft-rounded' colorScheme='blue' marginBottom='24px'>
							<TabList>
